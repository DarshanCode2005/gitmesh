export const externalTooltipHandler = (
  ...
  if (tooltip.body) {
    ...
    tooltip.body.forEach(({ lines, after }) => {
      lines.forEach((line) => {
        innerHtml += `<tr class="text-gray-200 text-xs font-medium"><td class="py-2"><span>${line}</span></td></tr>`;
      });
      ...
      after.forEach((a) => {
        if (a) {
          ...
          innerHtml += `
          <tr class="border-b border-gray-100 last:border-none text-gray-200 text-xs font-medium">
            <td class="pb-2">
